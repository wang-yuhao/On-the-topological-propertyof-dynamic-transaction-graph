...
for step in parameter_dict:
    ...
    for dataset_model in dataset_model_array:
        ...
        #betti_der_gain = 100 * (1 -rmse_betti__der_total.div(rmse_base_total.values))
        for i in range(12):
            path = "./performance/version_fixed_parameter/"
            perf = pd.concat([fl_gain.iloc[:, i], betti_gain.iloc[:,i]], axis=1).plot.bar()
