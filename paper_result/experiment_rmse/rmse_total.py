...
def arimax_base_rmse_mode(train_input, train_target, test_input, test_target):
    ...
    model  = pf.ARIMAX(data=train_input_diff_arr,formula="current_price~" + str(best_column),ar=1,ma=2,integ=0)
...
def run_print_model(train_input, train_target, test_input, test_target, train_days, test_days):
    ...
    return rf_base_rmse, xgbt_base_rmse, gp_base_rmse, enet_base_rmse, arimax_base_rmse
...
for step in parameter_dict:
    ...
    for window_size in window_size_array:
        ...
        for prediction_horizon in horizon_size_array:
            ...
            rf_base_rmse, xgbt_base_rmse, gp_base_rmse, enet_base_rmse, arimax_base_rmse = run_print_model(train_input, train_target, test_input, test_target, train_days, test_days)
            
            rmse = pd.DataFrame({'rf_base_rmse': [rf_base_rmse], 'xgbt_base_rmse': [xgbt_base_rmse], 'gp_base_rmse': [gp_base_rmse], 'enet_base_rmse': [enet_base_rmse], 'arimax_base_rmse': [arimax_base_rmse]})
        ...
        rmse_total.index = pd.Series(horizon_size_array)
        print("rmse_total:", rmse_total)    
        plt_5 = rmse_total.plot() 
        fig_5 = plt_5.get_figure()
        fig_5.savefig('WINDOW_' + str(window_size) + '_5.png')
        
        plt_4 = rmse_total.loc[:, 'rf_base_rmse':'enet_base_rmse' ].plot() 
        fig_4 = plt_4.get_figure()
        fig_4.savefig('WINDOW_' + str(window_size) + '_4.png')
